(() => {
  ...
  async function exportEvidence(_m,s){const locator=durable(senderLocator(s)),evidence=await Store.exportEvidenceByLocator(locator);return {ok:true,evidence:{...evidence,boot_id:BOOT_ID,recovery_ready:recoveryReady,recovery_failure:recoveryFailure?String(recoveryFailure.message||recoveryFailure):""}};}
