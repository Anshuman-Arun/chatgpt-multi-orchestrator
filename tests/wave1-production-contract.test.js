...
test("Wave-1 has one low-level DOM send actuator and does not synthesize Enter", () => {
  ...
  assert.equal((actuator.match(/\.click\(\)/g) || []).length, 1);
  assert.equal((actuator.match(/\.requestSubmit\(\)/g) || []).length, 1);
