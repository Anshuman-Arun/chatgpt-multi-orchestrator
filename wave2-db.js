((root, factory) => {
  ...
  else root.MultiAgentWave2Db = api;
  ...
  async function appendEvent(tx,input){const meta=os(tx,"meta"),events=os(tx,"events"),cur=await req(meta.get("journal_seq")),seq=Math.max(0,Number(cur?.value)||0)+1;meta.put({key:"journal_seq",value:seq});const d=input.delivery||{};const ev={seq,timestamp:Number(input.timestamp)||now(),event_type:String(input.event_type||"STATE_TRANSITION"),run_id:String(d.run_id||input.run_id||""),task_id:String(d.task_id||input.task_id||""),delivery_id:String(d.delivery_id||input.delivery_id||""),conversation_id:String(d.conversation_id||input.conversation_id||""),logical_agent_id:String(d.logical_agent_id||input.logical_agent_id||""),slot_id:String(d.slot_id||input.slot_id||""),previous_state:input.previous_state==null?null:String(input.previous_state),next_state:input.next_state==null?null:String(input.next_state),reason:String(input.reason||"").slice(0,240),lease_fence:Math.max(0,Number(input.lease_fence??d.lease_fence)||0),actor_id:String(input.actor_id||d.actor_id||"").slice(0,220),boot_id:String(input.boot_id||"").slice(0,220),evidence:compactEvidence(input.evidence)};events.add(ev);return ev;}
  async function readDelivery(tx,id){const d=await req(os(tx,"deliveries").get(String(id||"")));if(!d)throw new Error("Wave-2 Delivery was not found");return d;}
  function transition(d,next,at=now()){if(d.state===next)return d;if(!Core.canTransition(d.state,next))throw new Error(`Invalid Wave-2 Delivery transition ${d.state} -> ${next}`);return {...d,state:next,updated_at:at};}
  async function assertFence(tx,d,actor,fence,{fresh=true,at=now()}={}){const l=await req(os(tx,"leases").get(d.conversation_id));if(!l||String(l.owner_actor_id)!==String(actor||"")||Number(l.fence)!==Number(fence)||Number(d.lease_fence)!==Number(fence)){const e=new Error("Wave-2 sender lease fence is stale");e.code="wave2.lease_stale";throw e;}if(fresh&&Number(l.expires_at)<=at){const e=new Error("Wave-2 sender lease expired");e.code="wave2.lease_expired";throw e;}return l;}
  return Object.freeze({Core,DB_NAME,DB_VERSION,LEASE_MS,LEASE_RENEW_WINDOW_MS,STORE_NAMES,now,makeId,req,openDb,withTx,os,compactEvidence,appendEvent,readDelivery,transition,assertFence});
});
